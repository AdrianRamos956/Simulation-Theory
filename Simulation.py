...
class Simulation:
    ...
    def start(self):
        ...
        self.event_queue.put(
            SimEvent.SimEvent(sim_time + time_service, SimEvent.CUSTOMER_COMPLETE, current_lane, new_customer))
        ...
        while not self.event_queue.empty() and sim_time <= sim_end_time:
            ...
            if current_event.event_type == SimEvent.CUSTOMER_READY:
                ...
                self.event_queue.put(
                    SimEvent.SimEvent(sim_time, SimEvent.CUSTOMER_READY, customer_arrival, current_lane, new_customer))
                self.event_queue.put(
                    SimEvent.SimEvent(sim_time + time_service, SimEvent.CUSTOMER_COMPLETE, current_lane, new_customer))
